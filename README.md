...
- **จัดการฐานข้อมูลและสำรองข้อมูล (Full-Stack Backup & Restore)**:
  - **ครอบคลุม 7 ตารางฐานข้อมูล SQLite ครบ 100%**: บันทึกเวลาทำงาน (รวมรูปถ่ายหลักฐาน `attachmentUri`), กะการทำงานรายเดือน/ปี, วันหยุดนักขัตฤกษ์, ประวัติวันลา (รวมใบรับรองแพทย์), โควตาวันลา, กิจกรรมและนัดหมาย, และโน้ตช่วยจำ & To-Do List (รองรับการกู้คืนโน้ตแบบไม่มีหัวข้อได้สมบูรณ์)
  - **สำรองและกู้คืนการตั้งค่าระบบ (AsyncStorage Settings)**: ผนวกการตั้งค่านาฬิกาปลุกฉลาด (Smart Alarm Config), การเปิด/ปิดการสั่น (Haptics), และธีมของแอป เข้าไปในไฟล์สำรองโดยอัตโนมัติ
  - **ระบบกู้คืนข้อมูลแบบ Transaction ปลอดภัยสูง**: ดำเนินการผ่าน `db.withTransactionAsync` รับประกันความปลอดภัย หากไฟล์มีปัญหาจะ Rollback ข้อมูลเดิมทันที
  - ส่งออกเป็นไฟล์ `.json` พร้อมแชร์ออกภายนอก (Google Drive, LINE, Email) และนำเข้ากู้คืนได้ทั้งแบบแทนที่ (Replace) หรือรวมข้อมูล (Merge)
  - ล้างข้อมูลระบบพร้อมการยืนยันความปลอดภัยหลายชั้น (Multi-Step Safety Confirmation)
